...
$(document).ready(function() {
    ...
    window.trade = new c();
    window.trade.IN();
    $("body").append('<div id="back_to_top"><a href="#" id="back-to-top"><img src="'+base_url+'html/images/btt.png"></a></div>');
    $("#back_to_top").hide();
    $(function() {
    ...
    show_report = function(C) {
        var B = window.innerWidth;
        var D = window.innerHeight;
        var E = Math.round(0.9 * B);
        if (E > 620) {
            E = 620
        }
        var A = Math.round(0.9 * D) - 40;
        if (A > 710) {
            A = 710
        }
        var z = Math.round((B - E) / 2);
        var F = Math.round((D - A + 40) / 2);
        $("#report_form").css("left", z + "px");
        $("#report_form").css("top", F + "px");
        $("#report_form").css("width", E + "px");
        $("#report_form").css("height", A + "px");
        $("#report_close").css("left", parseInt(z + E - 40) + "px");
        $("#report_close").css("top", parseInt(F - 40) + "px");
        $("#report_close").css("display", "block");
        $("#report_overlay").show();
        $("#report_form").show();
        document.getElementById("report_iframe").src = C
    };
    var l = {};
    var o = {};
    var e = {};
    ...
    r_log = function(y) {
        var x = false;
        if (j.length > 0) {
            post_sclicks = JSON.stringify(jQuery.unique(j))
        } else {
            post_sclicks = ""
        }
        if (s.length > 0) {
            post_sclicks2 = JSON.stringify(jQuery.unique(s))
        } else {
            post_sclicks2 = ""
        }
        if (q.length > 0) {
            post_sviews = JSON.stringify(jQuery.unique(q))
        } else {
            post_sviews = ""
        }
        if (typeof ln != "undefined" && typeof lc != "undefined" && typeof ll != "undefined" && a == 1 && document.cookie.indexOf("nr=") == -1) {
            var w = [];
            var B = [];
            for (var A in o) {
                w[w.length] = A
            ...
            if (ln != "" && lc >= 0 && ll != "" && w.length > 0) {
                x = true;
                post_views = JSON.stringify(jQuery.unique(w));
                post_clicks = JSON.stringify(jQuery.unique(B));
                var z = "";
                if (typeof lq != "undefined" && lc == 0) {
                    z = lq
                }
    ...
    if (typeof $("#showMore") !== "undefined") {
        var i = '<div id="loading"><img src="'+base_url+'html/images/ajax-loader.gif" /></div>';
        var k = new Image();
        k.src = base_url + "html/images/ajax-loader.gif";
        var g = false;
        var u = 1;
    ...
    $("#topsearch").autocomplete({
        serviceUrl:site_url + "autocomplete?l=" + $("#topsearch").data("l"),
        paramName: "q",
        minChars: 2,
    ...
    $("#bottomsearch").autocomplete({
        serviceUrl: site_url + "autocomplete?l=" + $("#bottomsearch").data("l"),
        paramName: "q",
        minChars: 2,
        triggerSelectOnValidInput: false,
        onSelect: function(w) {
            ...
            b.loaded = !1, (c.attr("src") === d || c.attr("src") === !1) && c.attr("src", j.placeholder), c.one("appear", function() {
                if (!this.loaded) {
                    ...
                    a("<img />").bind("load", function() {
                        var d = site_url_thumbs_src + c.data(j.data_attribute) + '.jpg';
						//console.log(d);
                    ...
                    }).attr("src", site_url_thumbs_src + c.data(j.data_attribute) + '.jpg')
					.on('error', function() 
					{ 
                        var the_data_tn = c.data(j.data_attribute);
                        var new_data_tn = the_data_tn.replace(/_/g , "/"); 
                        var d2 = 'http://cdn.webclicks24.com.rsz.io/t/' + new_data_tn + '.jpg';
                        //var d2 = 'http://cdn.vid18.net/api/thumb/?u=' + the_data_tn ;//+ '.jpg';
                        c.attr("src", d2);
                        //console.log("error 3 = " + c.data(j.data_attribute) + " >> " + new_data_tn); 
					})
                }
