...
if(mobile_menu_list_count>0){if(window.innerWidth<=768){$('#mobile_menu_list').html(mobile_menu_list);}else{$('#desktop_menu_list').html(mobile_menu_list);}
$(window).on("resize",function(){if(window.innerWidth>768){if(!$('#desktop_menu_list').find("ul").length){$('#desktop_menu_list').append($('#mobile_menu_list ul'));$('#mobile_menu_list ul').remove();}}else{if(!$('#mobile_menu_list').find("ul").length){$('#mobile_menu_list').append($('#desktop_menu_list ul'));$('#desktop_menu_list ul').remove();}}});}}})}}
...
{basepath=basepath.replace(/\/+$/,'/');basepath=basepath.replace(/\/$/,'')+'default.htm';$("#rating .rating").hover(function(){var i=$(this).index();$("#rating .rating").addClass("rateon_forced_off");for(x=0;x<i+1;x++){$("#rating .rating:eq("+x+")").removeClass("rateon_forced_off")
...
{var language=window.navigator.userLanguage||window.navigator.language;for(it=0;it<langs.length;it++){var langre=new RegExp("^"+langs[it],'i');if(language.match(langre)){$.ageCheck(settings);break;}}}
$(document).on('ready',function(){var $block=$('.view_category');if($block.length){var scroll_top=$(window).scrollTop();var background_position=20;$(window).scroll(function(){var scroll=$(window).scrollTop();if(scroll>scroll_top&&($block.offset().top<$(window).scrollTop()+$(window).height()-50)){if(background_position<70){$block.css('background-position-x',parseInt(background_position+=2)+'px');}}else{if(background_position>20){$block.css('background-position-x',parseInt(background_position-=2)+'px');}}
scroll_top=scroll;});}
var flag_langs=false;$('.js-show-langs > a').on('click',function(){$(this).toggleClass('active');if(flag_langs==false){$(document).on('click',function(event){if(!$(event.target).closest($('.js-show-langs')).length){$('.js-show-langs > a').removeClass('active')
flag_langs=true;}});}
return false;});});
function linkInitSearch(lang,vm,algo_index)
{var $block_result=$('[data-block-results]');var $btn_close=$('[data-btn-close]');var $search_field=$('[name="index-search-form"]');var $active_blocks=$('#search_input, [data-block-results]');var clicked_link=false;$('body').on('click','[data-block-results] .url_link_container a',function(){clicked_link=true;});function showResults(){$block_result.show();$btn_close.show();$active_blocks.addClass('transform-translate');$('body').addClass('visibility-hidden');}
function hideResults(){var search_value=$search_field.val();if(search_value.length>1){if(clicked_link){_gaq.push(['_trackPageview','?q='+search_value]);_gaq.push(['_trackEvent','happy search','search',search_value]);}else{_gaq.push(['_trackEvent','sad search','search',search_value]);}
clicked_link=false;}
...
var link_order=(typeof content.hits[oindex].lang_order[lang])!="undefined"?content.hits[oindex].lang_order[lang]:content.hits[oindex].order;c_objects.push({id:content.hits[oindex].objectID,path:path,slug:content.hits[oindex].single_slug,title:title,order:link_order,unlisted:content.hits[oindex].unlisted,thumbnail:thumbnail,category_slug:content.hits[oindex].category.slug,category_title:category_title,category_title_original:content.hits[oindex].category.titles['title_en'],mobile_friendly:content.hits[oindex].mobile_friendly});}}
...
html+='</ul>';setTimeout(()=>{$('.ac-container .errors').html(html);},200);},reCenter(b){b.css('top',`${Math.max(0,(($(window).height()-(b.outerHeight()+150))/2))}px`);b.css('left',`${Math.max(0,(($(window).width()-b.outerWidth())/2))}px`);},buildHtml(){const copy=settings.copy;const months=['January','February','March','April','May','June','July','August','September','October','November','December'];let html='';html+='<div class="ac-overlay"></div>';html+='<div class="ac-container">';html+=`<h2>${settings.title}</h2>`;html+=`<p>${copy.replace('[21]',`<strong>${settings.minAge}</strong>`)}`;+'</p>';html+='<div class="errors"></div>';html+='<div class="fields"><select class="month">';for(let i=0;i<months.length;i++){html+=`<option value="${i}">${months[i]}</option>`;}
html+='</select>';html+='<input class="day" maxlength="2" placeholder="01" />';html+='<input class="year" maxlength="4" placeholder="1989"/>';html+='<button>Submit</button></div></div>';$('body').append(html);$('.ac-overlay').animate({opacity:0.8,},500,()=>{_this.reCenter($('.ac-container'));$('.ac-container').css({opacity:1,});});$('.ac-container .day, .ac-container .year').focus(function(){$(this).removeAttr('placeholder');});},setAge(){_this.age='';const birthday=new Date(_this.year,_this.month,_this.day);const ageDifMs=Date.now()-birthday.getTime();const ageDate=new Date(ageDifMs);_this.age=Math.abs(ageDate.getUTCFullYear()-1970);},setSessionStorage(key,val){try{sessionStorage.setItem(key,val);return true;}catch(e){return false;}},handleSuccess(){const successMsg=`<h3>${settings.successMsg.header}</h3><p>${settings.successMsg.body}</p>`;$('.ac-container').html(successMsg);setTimeout(()=>{$('.ac-container').animate({top:'-350px',},200,()=>{$('.ac-overlay').animate({opacity:'0',},500,()=>{if(settings.redirectTo!==''){window.location.replace(settings.redirectTo);}else{$('.ac-overlay, .ac-container').remove();}});});},2000);},handleUnderAge(){const underAgeMsg=`<h3>${settings.underAgeMsg}</h3>`;$('.ac-container').html(underAgeMsg);if(settings.redirectOnFail!==''){setTimeout(()=>{window.location.replace(settings.redirectOnFail);},2000);}},};if(sessionStorage.getItem('ageVerified')==='true'){return false;}
_this.buildHtml();$('.ac-container button').on('click',()=>{_this.setValues();if(_this.validate()===true){_this.setAge();if(_this.age>=settings.minAge){if(!_this.setSessionStorage('ageVerified','true')){console.log('sessionStorage not supported by your browser');}
_this.handleSuccess();}else{_this.handleUnderAge();}}});$(window).resize(()=>{_this.reCenter($('.ac-container'));setTimeout(()=>{_this.reCenter($('.ac-container'));},500);});};}(jQuery));